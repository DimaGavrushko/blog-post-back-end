...
router.post('/unlike', withAuth, async (req, res) => {
  try {
    if (req.userId !== req.body.userId) {
      ...
      const post = await Post.findOne({ _id: req.body.postId });
      post.likes = post.likes.filter(elem => !elem.equals(req.body.userId));
      await post.save();
      res.status(200).json(post);
    }
  } catch (e) {
...
router.post('/undislike', withAuth, async (req, res) => {
  try {
    if (req.userId !== req.body.userId) {
      ...
      const post = await Post.findOne({ _id: req.body.postId });
      post.dislikes = post.dislikes.filter(elem => !elem.equals(req.body.userId));
      await post.save();
      res.status(200).json(post);
    }
  } catch (e) {
...
router.post('/approve', withAuth, async (req, res) => {
  try {
    if (req.role !== 'admin') {
      ...
      await Post.updateOne({ _id: req.body.postId }, { isApproved: true });
    }
    res.status(200).json(await Post.findOne({ _id: req.body.postId }));
  } catch (e) {
    res.status(400).json({
      error: e.message
    });
